...
print("[Info] Setting up Coalescence configuration...")
...
mat_prop_dict = {"mat_E": 1.0, "mat_nu": 1.0 / 3.0, "w1": 1.0, "l0": 0.01}

domain_extrema = torch.tensor([[-0.5, 0.5], [-0.5, 0.5]])
crack_dict = {
    "x_init": [-0.25, -0.05, 0.15],
    "y_init": [-0.15, -0.05, 0.05],
    "L_crack": [0.14142, 0.14142, 0.14142],
    "angle_crack": [np.pi / 4, np.pi / 4, np.pi / 4],
}
...
coarse_mesh_file = "mesh/Coalescence_initial.msh"
fine_mesh_file = "mesh/Coalescence_initial.msh"
...
def setup_run_artifacts():
    ...
    model_path = PATH_ROOT / "result" / f"Coalescence_refine_{date}"
...
class FieldComputation(nn.Module):
    ...
    def fieldCalculation(self, inp):
        ...
        u = (
            (inp[:, -1] - y0) * (yL - inp[:, -1]) * disp_latent[:, 0]
            + (inp[:, -1] - y0) / (yL - y0) * torch.cos(self.theta)
        ) * self.lmbda
        v = (
            (inp[:, -1] - y0) * (yL - inp[:, -1]) * disp_latent[:, 1]
            + (inp[:, -1] - y0) / (yL - y0) * torch.sin(self.theta)
        ) * self.lmbda
...
def train():
    ...
    try:
        ...
        print("[NN Train] --- Starting Main Training (Continue Adaptive Mesh) ---")
        ...
        for j, disp_i in enumerate(disp):
            ...
            alpha_prev = pred_alpha
            hist_alpha = pred_alpha
